...
if [[ $HELP == "true" ]]; then
    ...
    echo "  -a  Use this option to clean up all namespaces, including IBM Cert Manager and Licensing."
    exit 0
fi
...
function delete_resource() {
    local RESOURCE_NAME=$1
    local NAMESPACE_NAME=$2
	if [[ "$RESOURCE_NAME" != crd* && "$RESOURCE_NAME" == *.* ]]; then
        crd_name=$(echo "$RESOURCE_NAME" | awk -F '/' '{print $1}')
        if ! ${CLI_CMD} get crd "${crd_name}" &>/dev/null; then
            return 0
        fi
    fi
    ${CLI_CMD} get "${RESOURCE_NAME}" -n "${NAMESPACE_NAME}" --ignore-not-found=true &>/dev/null
...
if [[ "$SEPARATION_DUTY" == "true" ]]; then
	...
	for RESOURCE in "${BAI_RESOURCES[@]}"; do
        # Retrieve resources in the BAI namespace
...
if [[ "$SEPARATION_DUTY" == "true" ]]; then
	...
	${CLI_CMD} delete project "${BAI_OPERATORS_NAMESPACE}"

	info "Wait until namespace ${BAI_OPERATORS_NAMESPACE} is completely deleted."
	count=0
	while :; do
		${CLI_CMD} get project "${BAI_OPERATORS_NAMESPACE}" 2>/dev/null
		if [[ $? -gt 0 ]]; then
			...
			if ((count <= 36)); then
				...
				${CLI_CMD} get project "${BAI_OPERATORS_NAMESPACE}" -o yaml
				exit 1
			fi
		fi
	done
	...
	${CLI_CMD} delete project "${BAI_SERVICES_NAMESPACE}"

	info "Wait until namespace ${BAI_SERVICES_NAMESPACE} is completely deleted."
	count=0
	while :; do
		${CLI_CMD} get project "${BAI_SERVICES_NAMESPACE}" 2>/dev/null
		if [[ $? -gt 0 ]]; then
			...
			if ((count <= 36)); then
				...
				${CLI_CMD} get project "${BAI_SERVICES_NAMESPACE}" -o yaml
				exit 1
			fi
		fi
	done
	...
	${CLI_CMD} delete project "${BAI_NAMESPACE}"

	info "Wait until namespace ${BAI_NAMESPACE} is completely deleted."
	count=0
	while :; do
		${CLI_CMD} get project "${BAI_NAMESPACE}" 2>/dev/null
		if [[ $? -gt 0 ]]; then
			...
			if ((count <= 36)); then
				...
				${CLI_CMD} get project "${BAI_NAMESPACE}" -o yaml
				exit 1
			fi
		fi
	done
