...
OPER_LIST=(
    "name:ibm-bai-foundation-operator"
    "name:ibm-bai-insights-engine-operator"
    )

# Below list will be used to track the different type of CR Kind types that need to be checked for while retrieving existing network policies
INSIGHTS_ENGINE_CR_KIND_MAPPING_LIST=( "Foundation" "InsightsEngine" )
...
function retrieve_network_policy_templates() {
    ...
    do
        ...
        # bai_operators_namespace is populated by the set_operator_operand_namespaces function

        pod_name=$(get_pod_name_by_label_name "${label_key}" "${label_value}" "${bai_operators_namespace}" )
        ...
        if [ -n "$pod_name" ]; then
            ...
            for (( i=0; i<len; i++ )); do
                ...
                if $CLI_CMD exec "$pod_name" -n "${bai_operators_namespace}" -- test -d "${operator_path_list[$i]}" >/dev/null 2>&1; then
                ...
                if [[ "$templates_present" == true ]]; then
                    ...
                    failed_copy=true
                    output=$($CLI_CMD -n ${bai_operators_namespace} cp ${pod_name}:${operator_path_list[$i]} ${target_template_path_list[$i]} 2>&1)
            ...
            if [[ "$template_copy_skipped" == true ]]; then
                warning "- Warning: Skipping the copy of generated network policies in the pod ${pod_name}.This is likely because ${pod_name} is not used by your deployment pattern."
            fi   
...
function retrieve_existing_network_policies() {
    ...
    #output=$(${CLI_CMD} get networkpolicy -n "${namespace}" -o json | jq -r --arg kind "$kind" '.items[] | select(.metadata.ownerReferences[]?.kind == $kind) | .metadata.name')
    #if [[ -n "$output" ]]; then
    #    echo "Retrieving existing network policies in the target namespace ${namespace} with the kind ${kind} ..."
    #    ${CLI_CMD} get networkpolicy -n "${namespace}" -o json | jq -r --arg kind "$kind" '
    #        .items[] 
    #        | select(.metadata.ownerReferences[]?.kind == $kind) 
    #        | .metadata.name' | while read -r netpol_name; do
    #        echo
    #        echo " - Retrieving network policy ${netpol_name} ..."
    #        ${CLI_CMD} get networkpolicy "${netpol_name}" -n "${namespace}" -o yaml > "${location}/${netpol_name}_${namespace}.yaml"
    #        if [[ $? -ne 0 ]]; then
    #            error " - Error: retrieving network policy ${netpol_name}."
    #            continue
    #        else
    #            echo " - Retrieved network policy ${netpol_name} ..."
    #        fi
    #
    #    done
    #else
    #    warning "There are no existing network policies in the target namespace ${namespace} with the kind ${kind} ..." 
    #fi
    ...
    if [[ ! -z "$np_names" ]]; then
        ...
        for np in $np_names; do
            ...
            kind=$(${CLI_CMD} get networkpolicy "$np" -n "$namespace" -o json | jq -r '.metadata.ownerReferences[0].kind // ""')
            matched=false
...
function set_operator_operand_namespaces(){
    ...
    if ${CLI_CMD} get configMap ibm-cp4ba-common-config -n $namespace >/dev/null 2>&1; then
        bai_services_namespace=$(${CLI_CMD} get configMap ibm-cp4ba-common-config -n $namespace --no-headers --ignore-not-found -o jsonpath='{.data.services_namespace}')
        bai_operators_namespace=$(${CLI_CMD} get configMap ibm-cp4ba-common-config -n $namespace --no-headers --ignore-not-found -o jsonpath='{.data.operators_namespace}')
    else
        warning "ibm-cp4ba-common-config configmap was not found in the project \"$namespace\"."
        warning " If the deployment being used has separate namespaces for operators and operands , please use the operands namespace while executing the bai-network-policies.sh script"
