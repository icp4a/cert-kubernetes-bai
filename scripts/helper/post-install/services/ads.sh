...
cp4baADSStatus()
{
  ...
  if [ -z ${adsBuildServiceDeployment_STATUS}  ]; then
    adsBuildServiceDeployment_STATUS="NotInstalled"
  fi
  echo "adsBuildServiceDeployment                   :  ${adsBuildServiceDeployment_STATUS}"

  local adsBuildServiceService_STATUS=`cat ${LOG_DIR}/ads-status.log | grep adsBuildServiceService | awk '{print $2}'`
  if [ -z ${adsBuildServiceService_STATUS}  ]; then
    adsBuildServiceService_STATUS="NotInstalled"
  fi
  echo "adsBuildServiceService                      :  ${adsBuildServiceService_STATUS}"
