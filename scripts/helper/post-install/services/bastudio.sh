...
cp4baBaStudioStatus()
{
  printHeaderMessage "CP4BA Service Status - BA Studio"
  ...
  #BAI
  #################################################
  BA_STUDIO_STATUS=`cat ${LOG_DIR}/bastudio-status.log | grep service | awk '{print $2}'`
  if [ -z ${BA_STUDIO_STATUS}  ]; then
    BA_STUDIO_STATUS="NotInstalled"
  fi
  echo "service:                                    :  ${BA_STUDIO_STATUS}"
}
...
cp4baBaStudioConsole()
{
  ...
  local BA_WORKPLACE_URL=`cat  ${LOG_DIR}/bastudio-access-info.log | grep "Business Automation Workplace"  | awk '{print $4}'`
