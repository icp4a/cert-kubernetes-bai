...
checkURL() {
  ...
  then
    ...
    then
      if [[ "$CAP" != "$CAPABILITY" ]]; then echo;printf '%b\n' "${BOLD_TEXT}#### ${CAPABILITY} ####${RESET_TEXT}";fi
      ...
      then printf '%b\n' "${RED_TEXT}Unauthorized${RESET_TEXT}: ${URL2}"
      else
      if [[ ! -z $(echo "$RET" | grep "404 Not Found") && -z $(echo "$RET" | grep -e "${CHECK_KEYWORD}") ]] || [[ ! -z $(echo "$RET" | grep "503 Service Unavailable") ]] || [[ ! -z $(echo "$RET" | grep "403 Forbidden") ]]
        then printf '%b\n' "${BOLD_TEXT}Not installed/Failed${RESET_TEXT}"
        else
          ...
          then
            ...
            then
              ...
              printf '%b\n' "Check in browser: ${BLUE_TEXT}${URL2}${RESET_TEXT}"
            else printf '%b\n' "${GREEN_TEXT}OK${RESET_TEXT}"
            fi
          else
            printf '%b\n' "${RED_TEXT}BAD${RESET_TEXT}: ${RED_TEXT}${URL2}${RESET_TEXT}"
	          if [[ "$VERBOSE" == "" ]]; then printf '%b\n' "Check in cmd: ${BLUE_TEXT}${CMD} | grep -e \"${CHECK_KEYWORD}\"${RESET_TEXT}";fi
          fi
        fi
      fi
      if [[ "$VERBOSE" == "-v" ]]; then printf '%b\n' "Check in cmd: ${BLUE_TEXT}${CMD}${RESET_TEXT} | grep -e \"${CHECK_KEYWORD}\"${RESET_TEXT}";fi
    fi
  fi
}
...
verifyRequiredData(){
  if [[ ! -e "./urlmap.csv" ]]
  then
    printf '%b\n' "${RED_TEXT}I cannot find urlmap.csv in running folder!${RESET_TEXT}"
    exit 1;
  fi
}
...
verifyInputParameters(){
  ...
  then
    ...
    echo
    printf '%b\n' "${BOLD_TEXT}Refer to the KC documentation for more details${RESET_TEXT}"
    exit 1
  fi
}
...
displayProductionNote() {
  ...
  then
    echo
    printf '%b\n' "${RED_TEXT}* Important note${RESET_TEXT}:"
    printf '%b\n' "You have to assign proper roles for ${USERNAME} in Access Control using ${BOLD_TEXT}admin${RESET_TEXT} on ${BLUE_TEXT}https://cpd-${NAMESPACE}.apps.${CLUSTER}${RESET_TEXT}"
    printf '%b\n' "To get ${BOLD_TEXT}admin${RESET_TEXT} password run:"
