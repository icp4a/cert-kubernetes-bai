...
function upgrade_deployment(){
    ...
    if [ ! -z $content_cr_name ]; then
        ...
        if [[ ${owner_ref} == "ICP4ACluster" ]]; then
            ...
            kubectl patch foundationrequest $foundationrequest_cr_name -n $project_name -p '{"spec":{"appVersion":"$BAI_RELEASE_BASE"}}' --type=merge >/dev/null 2>&1
            ...
            echo "STEP1:${RED_TEXT} # kubectl annotate content $content_cr_name kubectl.kubernetes.io/last-applied-configuration- -n $project_name${RESET_TEXT}"
            echo "STEP2:${RED_TEXT} # kubectl apply -f ${UPGRADE_DEPLOYMENT_CONTENT_CR} -n $project_name --force --overwrite=true${RESET_TEXT}"
    ...
    if [ ! -z $exist_wfps_cr_array ]; then
        ...
        do
            ...
            for ((retry=0;retry<=${maxRetry};retry++)); do
                isReady=$(kubectl get csv ibm-cp4a-wfps-operator.$BAI_CSV_VERSION -n $project_name -o jsonpath='{.status.phase}')
            ...
            kubectl apply -f ${UPGRADE_DEPLOYMENT_WFPS_CR} -n $project_name --force --overwrite=true >/dev/null 2>&1
            if [ $? -ne 0 ]; then
    ...
    if [ ! -z $icp4acluster_cr_name ]; then
       ...
       # Convert optional components array to list by common
        ...
        echo "${YELLOW_TEXT}[NEXT ACTION]${RESET_TEXT}:"
        echo "${YELLOW_TEXT}Apply custom resource follow below command manually (If you use image tag, you need to update to use new tag manually before apply).${RESET_TEXT}"
        echo "STEP1: # kubectl annotate icp4acluster $icp4acluster_cr_name kubectl.kubernetes.io/last-applied-configuration- -n $project_name"
        echo "STEP2: # kubectl apply -f ${UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR} -n $project_name --force --overwrite=true"
        echo "${YELLOW_TEXT}How to check the overall upgrade status for CP4BA/zenService/IM${RESET_TEXT}"
