...
function upgrade_deployment(){
    ...
    if [ ! -z $content_cr_name ]; then
        ...
        if [[ ${owner_ref} == "ICP4ACluster" ]]; then
            ...
            ${CLI_CMD} get $cr_type $content_cr_name -n $project_name -o yaml > ${UPGRADE_DEPLOYMENT_CONTENT_CR_TMP}
            
            # Update the appVersion in foundationrequest
            foundationrequest_cr_name=$(${CLI_CMD} get foundationrequest -n $project_name --no-headers --ignore-not-found | awk '{print $1}')
            ${CLI_CMD} patch foundationrequest $foundationrequest_cr_name -n $project_name -p '{"spec":{"appVersion":"$BAI_RELEASE_BASE"}}' --type=merge >/dev/null 2>&1
            ...
            info "Merging existing CP4BA Content Custom Resource with new version ($BAI_RELEASE_BASE)"
            ...
            ${SED_COMMAND} "s|release: .*|release: ${BAI_RELEASE_BASE}|g" ${UPGRADE_DEPLOYMENT_CONTENT_CR_TMP}
            ${SED_COMMAND} "s|appVersion: .*|appVersion: ${BAI_RELEASE_BASE}|g" ${UPGRADE_DEPLOYMENT_CONTENT_CR_TMP}
            ...
            if (( $css_instance_number > 0  )); then
                ...
                do
                    ${CLI_CMD} scale --replicas=0 deployment ${cr_metaname}-css-deploy-${j} -n $project_name >/dev/null 2>&1
                done
            fi

            info "Scaling down CPE deployment"
            ${CLI_CMD} scale --replicas=0 deployment ${cr_metaname}-cpe-deploy -n $project_name >/dev/null 2>&1
            echo "Done!"
            info "Scaling down Navigator deployment"
            ${CLI_CMD} scale --replicas=0 deployment ${cr_metaname}-navigator-deploy -n $project_name >/dev/null 2>&1
            echo "Done!"
            ...
            ${CLI_CMD} scale --replicas=1 deployment ibm-content-operator -n $project_name >/dev/null 2>&1
            if [ $? -eq 0 ]; then
                sleep 1
                echo "Done!"
            else
            ...
            ${CLI_CMD} scale --replicas=1 deployment icp4a-foundation-operator -n $project_name >/dev/null 2>&1
            if [ $? -eq 0 ]; then
                sleep 1
                echo "Done!"
            else
            ...
            info "Scaling up \"IBM Business Automation Insights stand-alone (CP4BA) multi-pattern\" operator to install zenService"
            ${CLI_CMD} scale --replicas=1 deployment ibm-cp4a-operator -n $project_name >/dev/null 2>&1
            if [ $? -eq 0 ]; then
                sleep 1
                echo "Done!"
            else
            ...
            info "The new version ($BAI_RELEASE_BASE) of CP4BA Content Custom Resource is created ${UPGRADE_DEPLOYMENT_CONTENT_CR}"
            printf "\n"
            echo "${YELLOW_TEXT}[NEXT ACTION]:${RESET_TEXT}"
            echo "${YELLOW_TEXT}Apply custom resource follow below command manually (If you use image tag, you need to update to use new tag manually before apply).${RESET_TEXT}"
            echo "STEP1:${RED_TEXT} # ${CLI_CMD} annotate content $content_cr_name kubectl.kubernetes.io/last-applied-configuration- -n $project_name${RESET_TEXT}"
            echo "STEP2:${RED_TEXT} # ${CLI_CMD} apply -f ${UPGRADE_DEPLOYMENT_CONTENT_CR} -n $project_name --force --overwrite=true${RESET_TEXT}"
            echo "${YELLOW_TEXT}How to check the overall upgrade status for CP4BA/zenService/IM${RESET_TEXT}"
            echo "STEP3: ${RED_TEXT}# ./bai-deployment.sh -m upgradeDeploymentStatus -n $TARGET_PROJECT_NAME${RESET_TEXT}"
            printf "\n"
            echo "${YELLOW_TEXT}[ATTENTION]: The zenService will be ready in about 30 minutes after the new version ($BAI_RELEASE_BASE) of CP4BA custom resource was applied.${RESET_TEXT}"
            ...
            #     printf "\n"
            #     # echo "${YELLOW_TEXT}[NEXT ACTION]${RESET_TEXT}: "
            #     # msgB "Run \"bai-deployment.sh -m upgradeDeploymentStatus -n $project_name\" to get overview upgrade status for CP4BA"
            # fi
        fi
    fi
    ...
    if [ ! -z $exist_wfps_cr_array ]; then
        ...
        do
            ...
            cr_type="WfPSRuntime"
            cr_metaname=$(${CLI_CMD} get $cr_type ${item} -n $project_name -o yaml | ${YQ_CMD} r - metadata.name)
            ...
            ${CLI_CMD} get $cr_type ${item} -n $project_name -o yaml > ${UPGRADE_DEPLOYMENT_WFPS_CR_TMP}
            
            # Backup existing WfPSRuntime CR
            mkdir -p ${UPGRADE_DEPLOYMENT_CR_BAK}
            ${COPY_CMD} -rf ${UPGRADE_DEPLOYMENT_WFPS_CR_TMP} ${UPGRADE_DEPLOYMENT_WFPS_CR_BAK}

            info "Merging existing IBM CP4BA Workflow Process Service custom resource: \"${item}\" with new version ($BAI_RELEASE_BASE)"
            # Delete unnecessary section in CR
            ...
            success "Completed to merge existing IBM CP4BA Workflow Process Service custom resource with new version ($BAI_RELEASE_BASE)"

            info "Scaling up \"IBM CP4BA Workflow Process Service\" operator"
            ${CLI_CMD} scale --replicas=1 deployment ibm-cp4a-wfps-operator -n $project_name >/dev/null 2>&1
            if [ $? -eq 0 ]; then
                sleep 1
                echo "Done!"
            else
            ...
            for ((retry=0;retry<=${maxRetry};retry++)); do
                isReady=$(${CLI_CMD} get csv ibm-cp4a-wfps-operator.$BAI_CSV_VERSION -n $project_name -o jsonpath='{.status.phase}')
                ...
                if [[ $isReady != "Succeeded" ]]; then
                    ...
                    printf "\n"
                    echo -e "\x1B[1mCheck the status of ReplicaSet issuing the following command:\x1B[0m"
                    ...
                    pod_name=$(${CLI_CMD} get pod -l=name=ibm-cp4a-wfps-operator -n $project_name -o 'custom-columns=NAME:.metadata.name,PHASE:.status.phase,READY:.status.containerStatuses[0].ready,DELETED:.metadata.deletionTimestamp' --no-headers | grep 'Running' | grep 'true' | grep '<none>' | head -1 | awk '{print $1}')
                    if [ -z $pod_name ]; then
            ...
            info "Apply the new version ($BAI_RELEASE_BASE) of IBM CP4BA Workflow Process Service custom resource"
            ${CLI_CMD} annotate WfPSRuntime ${item} kubectl.kubernetes.io/last-applied-configuration- -n $project_name >/dev/null 2>&1
            ${CLI_CMD} apply -f ${UPGRADE_DEPLOYMENT_WFPS_CR} -n $project_name --force --overwrite=true >/dev/null 2>&1
            if [ $? -ne 0 ]; then
                ...
                printf "\n"
                # echo "${YELLOW_TEXT}[NEXT ACTION]${RESET_TEXT}:"
                # msgB "Run \"bai-deployment.sh -m upgradeDeploymentStatus -n $project_name\" to get overview upgrade status for IBM CP4BA Workflow Process Service"
            fi
        done
    fi
    ...
    if [ ! -z $icp4acluster_cr_name ]; then
        ...
        # fi
        info "Merging existing CP4BA Custom Resource with new version ($BAI_RELEASE_BASE)"
        # Delete unnecessary section in CR
        ...
        # replace release/appVersion
        ${SED_COMMAND} "s|release: .*|release: ${BAI_RELEASE_BASE}|g" ${UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR_TMP}
        ${SED_COMMAND} "s|appVersion: .*|appVersion: ${BAI_RELEASE_BASE}|g" ${UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR_TMP}
        ...
        if [[ (" ${EXISTING_PATTERN_ARR[@]} " =~ "workflow") || (" ${EXISTING_PATTERN_ARR[@]} " =~ "workflow-process-service") ]]; then
            ...
            # Workflow authoring/WfPS authoring use embedded PFS starting from $BAI_RELEASE_BASE
            ...
            ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/pfs_configuration"}]' >/dev/null 2>&1
            ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/baw_configuration/0/pfs_bpd_database_init_job"}]' >/dev/null 2>&1
            ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/pfs_bpd_database_init_job"}]' >/dev/null 2>&1
            ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/kafka_services"}]' >/dev/null 2>&1
        fi
       ...
       # Convert optional components array to list by common
        ...
        # Change ssl_protocol for PFS required in $BAI_RELEASE_BASE release
        ...
        # Support multiple tos instance from $BAI_RELEASE_BASE
        if [[ " ${EXISTING_OPT_COMPONENT_ARR[@]} " =~ "baw_authoring" ]]; then
            ...
            if [[ ! -z "$baw_instance_flag" ]]; then
                ...
                ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/case/object_store_name_tos"}]' >/dev/null 2>&1
                ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/case/connection_point_name_tos"}]' >/dev/null 2>&1
                ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/case/datasource_name_tos"}]' >/dev/null 2>&1
                ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/case/target_environment_name"}]' >/dev/null 2>&1
                ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/case/desktop_name"}]' >/dev/null 2>&1
            fi
        fi
        ...
        if [[ (! " ${EXISTING_OPT_COMPONENT_ARR[@]} " =~ "baw_authoring") && " ${EXISTING_PATTERN_ARR[@]} " =~ "workflow" ]]; then
            ...
            while true; do
                ...
                if [[ ! -z "$baw_instance_flag" ]]; then
                    ...
                    if [[ (-z $baw_connection_point_name_tos || -z $baw_object_store_name_tos) && (-z $init_section) ]]; then    
                        warning "Not found both baw_configuration.[0].case.connection_point_name_tos/object_store_name_tos and oc_cpe_obj_store_workflow_pe_conn_point_name under initialize_configuration, refer KC  https://www.ibm.com/docs/en/cloud-paks/cp-biz-automation/$BAI_RELEASE_BASE?topic=deployment-upgrading-business-automation-workflow-runtime"
                    fi
                    if [[ ! -z "$baw_object_store_name_tos" ]]; then
            ...
            while true; do
                baw_instance_flag=`cat $UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR_TMP | ${YQ_CMD} r - spec.baw_configuration.[${baw_instance_index}].case`
                if [[ ! -z "$baw_instance_flag" ]]; then
                    ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/baw_configuration/${baw_instance_index}/case/object_store_name_tos"}]' >/dev/null 2>&1
                    ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/baw_configuration/${baw_instance_index}/case/connection_point_name_tos"}]' >/dev/null 2>&1
                    ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/baw_configuration/${baw_instance_index}/case/datasource_name_tos"}]' >/dev/null 2>&1
                    ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/baw_configuration/${baw_instance_index}/case/target_environment_name"}]' >/dev/null 2>&1
                    ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/baw_configuration/${baw_instance_index}/case/desktop_name"}]' >/dev/null 2>&1
                    ((baw_instance_index++))
                else
                    break
                fi
        ...
        success "Completed to merge existing CP4BA Custom Resource with new version ($BAI_RELEASE_BASE)"
        # info "Remove initialize_configuration/verify_configuration from CP4BA Custom Resource"
        ...
        #     EXISTING_OPT_COMPONENT_ARR=( "${EXISTING_OPT_COMPONENT_ARR[@]}" "pfs" )
        # fi
        # Workflow authoring/runtime and WfPS authoring use embedded PFS starting from $BAI_RELEASE_BASE
        ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/pfs_configuration"}]' >/dev/null 2>&1
        ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/elasticsearch_configuration"}]' >/dev/null 2>&1
        ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/baw_configuration/0/pfs_bpd_database_init_job"}]' >/dev/null 2>&1
        ${CLI_CMD} patch icp4acluster $icp4acluster_cr_name -n $project_name --type=json -p='[{"op": "remove", "path": "/spec/workflow_authoring_configuration/pfs_bpd_database_init_job"}]' >/dev/null 2>&1
        # fi
        ...
        info "The new version ($BAI_RELEASE_BASE) of CP4BA Custom Resource is created ${UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR}"
        printf "\n"
        echo "${YELLOW_TEXT}[NEXT ACTION]${RESET_TEXT}:"
        echo "${YELLOW_TEXT}Apply custom resource follow below command manually (If you use image tag, you need to update to use new tag manually before apply).${RESET_TEXT}"
        echo "STEP1: # ${CLI_CMD} annotate icp4acluster $icp4acluster_cr_name kubectl.kubernetes.io/last-applied-configuration- -n $project_name"
        echo "STEP2: # ${CLI_CMD} apply -f ${UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR} -n $project_name --force --overwrite=true"
        echo "${YELLOW_TEXT}How to check the overall upgrade status for CP4BA/zenService/IM${RESET_TEXT}"
        echo "STEP3: # ./bai-deployment.sh -m upgradeDeploymentStatus -n $TARGET_PROJECT_NAME"
        printf "\n"
        echo "${YELLOW_TEXT}[ATTENTION]: The zenService will be ready in about 30 minutes after the new version ($BAI_RELEASE_BASE) of CP4BA custom resource was applied.${RESET_TEXT}"
        printf "\n"
