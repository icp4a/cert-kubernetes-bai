...
spec: {}
EOF
    isOperatorGrp=$(kubectl get operatorgroup --all-namespaces --no-headers --ignore-not-found | grep ibm-cert-manager)
...
EOF
    ...
    local maxRetry=30
    info "Checking if IBM Cert-manager Operator is ready"
