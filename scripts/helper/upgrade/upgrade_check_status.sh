...
function check_cp4ba_operator_version(){
    ...
    for ((retry=0;retry<=${maxRetry};retry++)); do
        cp4a_operator_csv_name=$(kubectl get csv -n $project_name --no-headers --ignore-not-found | grep "IBM Business Automation Insights stand-alone (CP4BA) multi-pattern" | awk '{print $1}')
        ...
        if [[ "$cp4a_operator_csv_version" == "${BAI_CSV_VERSION//v/}" ]]; then
            success "The current IBM Cloud Pak® for Business Automation Operator is already ${BAI_CSV_VERSION//v/}"
            break
            # exit 1
        ...
        elif [[ "$cp4a_operator_csv_version" != "${BAI_CSV_VERSION//v/}" ]]; then
...
function check_operator_status(){
    ...
    if [[ "$check_mode" == "full" ]]; then
        ...
        for ((retry=0;retry<=${maxRetry};retry++)); do
            isReady=$(kubectl get csv ibm-cp4a-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
            # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
            if [[ -z $isReady ]]; then
                fail "Failed to upgrade the IBM Business Automation Insights stand-alone (CP4BA) multi-pattern operator to ibm-cp4a-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                ...
                if [[ "$check_channel" != "channel" ]]; then
                    ...
                    if [ -z $pod_name ]; then
                        error "IBM Business Automation Insights stand-alone (CP4BA) multi-pattern Operator pod is NOT running"
                        CHECK_CP4BA_OPERATOR_RESULT=( "${CHECK_CP4BA_OPERATOR_RESULT[@]}" "FAIL" )
                        break
                    else
                        success "IBM Business Automation Insights stand-alone (CP4BA) multi-pattern Operator is running"
                        info "Pod: $pod_name"
                        CHECK_CP4BA_OPERATOR_RESULT=( "${CHECK_CP4BA_OPERATOR_RESULT[@]}" "PASS" )
                        break
                    fi
                elif [[ "$check_channel" == "channel" ]]; then
                    success "IBM Business Automation Insights stand-alone (CP4BA) multi-pattern Operator is in the phase of \"$isReady\"!"
                    CHECK_CP4BA_OPERATOR_RESULT=( "${CHECK_CP4BA_OPERATOR_RESULT[@]}" "PASS" )
                    break
                fi
            fi
    ...
    for ((retry=0;retry<=${maxRetry};retry++)); do
        isReady=$(kubectl get csv ibm-content-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
        # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
        if [[ -z $isReady ]]; then
            csv_version=""
            ...
            if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                if [[ $retry -eq ${maxRetry} ]]; then
                    fail "Failed to upgrade the IBM CP4BA FileNet Content Manager operator to ibm-content-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                    msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                    exit 1
                else
                    sleep 30
    ...
    for ((retry=0;retry<=${maxRetry};retry++)); do
        isReady=$(kubectl get csv icp4a-foundation-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
        # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
        if [[ -z $isReady ]]; then
            csv_version=""
            ...
            if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                if [[ $retry -eq ${maxRetry} ]]; then
                    fail "Failed to upgrade the IBM CP4BA Foundation operator to icp4a-foundation-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                    msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                    exit 1
                else
                    sleep 30
            ...
            if [[ "$check_channel" != "channel" ]]; then
                pod_name=$(kubectl get pod -l=name=icp4a-foundation-operator,release=$BAI_RELEASE_BASE --no-headers --ignore-not-found -n $project_name -o 'custom-columns=NAME:.metadata.name,PHASE:.status.phase,READY:.status.containerStatuses[0].ready,DELETED:.metadata.deletionTimestamp' | grep 'Running' | grep 'true' | grep '<none>' | head -1 | awk '{print $1}')
                if [ -z $pod_name ]; then
    ...
    for ((retry=0;retry<=${maxRetry};retry++)); do
        isReady=$(kubectl get csv ibm-ads-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
        # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
        if [[ -z $isReady ]]; then
            csv_version=""
            ...
            if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                if [[ $retry -eq ${maxRetry} ]]; then
                    fail "Failed to upgrade the IBM CP4BA Automation Decision Service operator to ibm-ads-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                    msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                    exit 1
                else
                    sleep 30
    ...
    if [[ "$check_mode" == "full" ]]; then
        ...
        for ((retry=0;retry<=${maxRetry};retry++)); do
            isReady=$(kubectl get csv ibm-odm-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
            # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
            if [[ -z $isReady ]]; then
                csv_version=""
                ...
                if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                    if [[ $retry -eq ${maxRetry} ]]; then
                        fail "Failed to upgrade the IBM Operational Decision Manager operator to ibm-odm-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                        msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                        exit 1
                    else
                        sleep 30
    ...
    if [[ "$check_mode" == "full" ]]; then
        ...
        if [[ "$arch_type" == "amd64" ]]; then
            ...
            for ((retry=0;retry<=${maxRetry};retry++)); do
                isReady=$(kubectl get csv ibm-dpe-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
                # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
                if [[ -z $isReady ]]; then
                    csv_version=""
                    ...
                    if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                        if [[ $retry -eq ${maxRetry} ]]; then
                            fail "Failed to upgrade the IBM Document Processing Engine operator to ibm-dpe-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                            msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                            exit 1
                        else
                            sleep 30
    ...
    for ((retry=0;retry<=${maxRetry};retry++)); do
        isReady=$(kubectl get csv ibm-cp4a-wfps-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
        if [[ -z $isReady ]]; then
            csv_version=""
            ...
            if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                if [[ $retry -eq ${maxRetry} ]]; then
                    fail "Failed to upgrade the IBM CP4BA Workflow Process Service operator to ibm-cp4a-wfps-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                    msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                    exit 1
                else
                    sleep 30
    ...
    if [[ "$check_mode" == "full" ]]; then
        ...
        for ((retry=0;retry<=${maxRetry};retry++)); do
            isReady=$(kubectl get csv ibm-insights-engine-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
            # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
            if [[ -z $isReady ]]; then
                csv_version=""
                ...
                if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                    if [[ $retry -eq ${maxRetry} ]]; then
                        fail "Failed to upgrade the IBM CP4BA Insights Engine operator to ibm-insights-engine-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                        msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                        exit 1
                    else
                        sleep 30
    ...
    for ((retry=0;retry<=${maxRetry};retry++)); do
        isReady=$(kubectl get csv ibm-pfs-operator.$BAI_CSV_VERSION --no-headers --ignore-not-found -n $project_name -o jsonpath='{.status.phase}')
        # isReady=$(kubectl exec $cpe_pod_name -c ${meta_name}-cpe-deploy -n $project_name -- cat /opt/ibm/version.txt |grep -F "P8 Content Platform Engine 23.0.1")
        if [[ -z $isReady ]]; then
            csv_version=""
            ...
            if [[ "v$csv_version" != $BAI_CSV_VERSION ]]; then
                if [[ $retry -eq ${maxRetry} ]]; then
                    fail "Failed to upgrade the IBM CP4BA Process Federation Server operator to ibm-pfs-operator.$BAI_CSV_VERSION under project \"$project_name\"" 
                    msg "Check the Subscription and ClusterServiceVersions and then fix issue firstly."
                    exit 1
                else
                    sleep 30
