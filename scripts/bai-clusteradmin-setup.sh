...
function get_entitlement_registry(){
    ...
    while true; do
        if [[ ! -z "$BAI_AUTO_ENTITLEMENT_KEY" && ! -z "$BAI_AUTO_LOCAL_REGISTRY" ]]; then
            printf '%b\n' "\x1B[1;31mSet one the following environment variables [BAI_AUTO_ENTITLEMENT_KEY] or [BAI_AUTO_LOCAL_REGISTRY]\x1B[0m"
            printf '%b\n' "Exiting..."
            exit 1
        fi
...
function check_airgap_mode(){
    ...
    if [ -z "$BAI_AUTO_AIGRAP_MODE" ]; then
        COLUMNS=12
        printf '%b\n' "\x1B[1mWould you like to set up the cluster for an online based IBM Business Automation Insights deployment or for an airgap/offline based IBM Business Automation Insights deployment: \x1B[0m"
    ...
    if [[ $NON_ADMIN == "false" ]]; then
        if [ -z "$BAI_AUTO_CLUSTER_USER" ]; then
            ...
            if [ "$returnValue" == 1 ]; then
                ...
                printf '%b\n' "\x1B[33;5mATTENTION: \x1B[0m\x1B[1mWhen you run bai-deployment.sh script, Use the cluster admin user.\n\x1B[0m"
                sleep 5
            else
...
function check_storage_class() {
    ...
    then
       ...
       printf "\n"
       printf '%b\n' "\x1B[1mUse available storage classes.\x1B[0m"
    fi
