...
## To fix the Error message in the bai-cp4a-clusteradmin.sh script for kubectl - https://jsw.ibm.com/browse/DBACLD-180253
...
function prompt_wfps_license(){
    ...
    printf '%b\n' "\x1B[1;31mhttps://www.ibm.com/support/customer/csol/terms/?id=L-ACQV-MS7LQZ&lc=en\n\x1B[0m"
...
function create_project() {
    ...
    then
        ...
        if [ $isProjExists -ne 2 ] ; then
            ...
            printf '%b\n' "\x1B[1mNamespace \"${project_name}\" already exists! Continue...\x1B[0m"
        fi
    fi
...
function verify_existing_csv(){
    ...
    #${CLI_CMD} get csv --all-namespaces|grep ibm-bai-insights-engine-operator.v 
...
function retrieve_domain_name(){
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        if [[ -z "$OTHER_PLATFORM_TYPE_DOMAIN" ]]; then
            ...
            if ping -c 1 -W 2 "$OTHER_PLATFORM_TYPE_DOMAIN" >&3 2>&3; then
