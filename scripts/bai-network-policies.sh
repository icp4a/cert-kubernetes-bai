...
function show_help() {
    ...
    echo "Usage:"
    echo
    echo "bai-network-policies.sh -m [modeType] -n [bai_namespace]"
    echo
    echo "Options:"
    echo
    echo "  -h  Display help"
    ...
    echo
    echo "  -n  Required: The target namespace of the BAI deployment."
    echo "                If BAI Standalone is deployed using separate namespaces for operators and operands/services, the value is the namespace where BAI Standalone operands/services are deployed."
    echo
...
if [[ -z "$TARGET_PROJECT_NAME" ]]; then
    echo -e "\x1B[1;31mPlease rerun command and include value for \"-n <BAI_NAMESPACE>\" option.\n\x1B[0m"
    show_help
    exit 1
fi
...
namespaces_used=("$bai_services_namespace" "$bai_operators_namespace")
netpol_oper_path_list=("/tmp/${bai_services_namespace}/network-policies" "/tmp/${bai_operators_namespace}/network-policies")
netpol_targ_path_list=("${CUR_DIR}/network-policies/${bai_services_namespace}" "${CUR_DIR}/network-policies/${bai_operators_namespace}")
netpol_targ_log_path=network-policies/${bai_services_namespace}/logs  # save_log function wants relative path
...
if [ "$RUNTIME_MODE" == "generate" ]; then
    ...
    echo "${GREEN_TEXT}---------------------------------${RESET_TEXT}"
    echo
    echo "${RED_TEXT}IMPORTANT: ${YELLOW_TEXT}Before generating the network policy templates, please confirm you have set the property 'shared_configuration.sc_generate_sample_network_policies' to 'true' in your BAI custom resource and your BAI deployment has completed successfully. ${RESET_TEXT}"
...
if [ "$RUNTIME_MODE" == "install" ]; then
    ...
    for (( i=0; i<len; i++ )); do
        ...
        if [[ -d "${netpol_targ_template_path_list[$i]}" ]]; then
            ...
            echo
            warning "The script did not find any generated network policy templates in the folder ${netpol_targ_template_path_list[$i]}.This could either mean you must execute \"bai-network-policies.sh\" in the \"generate\" mode first or that all required network policy templates have been generated and saved in another folder. "
        fi
    done
