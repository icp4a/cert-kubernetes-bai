...
if [ "$RUNTIME_MODE" == "upgradeOperator" ]; then
    ...
    if [[ "$PLATFORM_SELECTED" == "others" ]]; then
        ...
        if [[ $READY_FOR_DIRECT_UPGRADE == "Yes" ]]; then
            ...
            if [[ "$bai_operator_csv_version" == "24.0."* || "$bai_operator_csv_version" == "24.1."* ]]; then
        ...
        # CPFS 4.19.2 upgrade issue — patch operandConfig common-service to set
        # spec.services[].resources[].data.spec.primaryUpdateMethod=restart on the common-service-db
        # PostgreSQL Cluster resource so CPFS can complete the upgrade without a rolling-update failure.
        # This is applied immediately after setup_tenant.sh completes so CPFS picks it up on its
        ...
        if ${CLI_CMD} get operandconfig common-service -n "$cpfs_services_ns" --no-headers --ignore-not-found >&3 2>&3; then
            _cpfs_patched=$(${CLI_CMD} get operandconfig common-service -n "$cpfs_services_ns" -o yaml | \
              ${CPFS_YQ_PATH} '(.spec.services[] | select(.resources) | .resources[] | select(.apiVersion == "pg.ibm.com/v1" and .kind == "Cluster" and .name == "common-service-db") | .data.spec.primaryUpdateMethod) = "restart"')
            if [[ -n "$_cpfs_patched" ]]; then
                ...
                if [[ $? -eq 0 ]]; then
                    ...
                    echo "  ${CLI_CMD} get operandconfig common-service -n $cpfs_services_ns -o yaml | ${CPFS_YQ_PATH} '(.spec.services[] | select(.resources) | .resources[] | select(.apiVersion == \"pg.ibm.com/v1\" and .kind == \"Cluster\" and .name == \"common-service-db\") | .data.spec.primaryUpdateMethod) = \"restart\"' | ${CLI_CMD} apply -f -"
                fi
            else
        ...
        zen_edb_parm1="DATABASE_ENABLE_SSL"
        zen_edb_parm1_value="true"
        zen_edb_parm2="DATABASE_SSL_MODE"
        zen_edb_parm2_value="require"
        #zen_edb_cfg_found=$(${CLI_CMD} get configmap $zen_edb_cfg --no-headers | head -1 | awk '{print $1}' -n $BAI_SERVICES_NS )
        if ${CLI_CMD} get configmap $zen_edb_cfg -o jsonpath='{.data.$zen_edb_parm1}' &> /dev/null && ${CLI_CMD} get configmap $zen_edb_cfg -o jsonpath='{.data.$zen_edb_parm2}' -n $BAI_SERVICES_NS &> /dev/null; then
            echo "Required parameters exist in the $zen_edb_cfg configmap"
            edb_parm_result="true"
        else
            echo "Required Zen parameters does not exist in the $zen_edb_cfg configmap"
            edb_parm_result="false"
        fi

        # Patch configmap if the required parameters does not exist.
        if [ "$edb_parm_result" = "false" ]; then
           ${CLI_CMD} patch configmap $zen_edb_cfg -n $BAI_SERVICES_NS --type=merge -p '{"data": {"'$zen_edb_parm1'":"'$zen_edb_parm1_value'","'$zen_edb_parm2'":"'$zen_edb_parm2_value'"}}' 
        fi  
...
############## End - Migration CPfs mode and upgrade BAIS Operators ##############
...
if [[ "$RUNTIME_MODE" == "upgradeDeploymentStatus" ]]; then
    ...
    # at 71% due to the common-service-db PostgreSQL primary update failing with switchover.
    # Patching operandConfig here ensures the fix is in place before zen monitoring begins.
