            ...
            while true; do
                ...
                "1")
                    if [[ $DEPLOYMENT_WITH_PROPERTY == "No" ]]; then
                        ...
                        info "Run bai-prerequisites.sh to the modify platform type"
                        prompt_press_any_key_to_continue
                    fi
                    break
                    ;;
...
if [ "$RUNTIME_MODE" == "upgradeOperator" ]; then
    ...
    sub_inst_list=$(${CLI_CMD} get subscription.operators.coreos.com -n $TARGET_PROJECT_NAME|grep ibm-bai-insights-engine-operator|awk '{if(NR>0){if(NR==1){ arr=$1; }else{ arr=arr" "$1; }} } END{ print arr }')
    ...
    for i in ${!sub_array[@]}; do
        if [[ ! -z "${sub_array[i]}" ]]; then
            if [[ ${sub_array[i]} = ibm-bai-operator* ]]; then
                current_version=$(${CLI_CMD} get subscription.operators.coreos.com ${sub_array[i]} --no-headers --ignore-not-found -n $TARGET_PROJECT_NAME -o 'jsonpath={.status.currentCSV}') >&3 2>&3
                installed_version=$(${CLI_CMD} get subscription.operators.coreos.com ${sub_array[i]} --no-headers --ignore-not-found -n $TARGET_PROJECT_NAME -o 'jsonpath={.status.installedCSV}') >&3 2>&3
    ...
    if [[ "$PLATFORM_SELECTED" == "others" ]]; then
        ...
        # Create ibm-bai-shared-info configMap if not exist
        ...
        if [ $ENABLE_PRIVATE_CATALOG -eq 1 ]; then
            ...
            for i in ${!sub_array[@]}; do
                if [[ ! -z "${sub_array[i]}" ]]; then
                    if [[ ${sub_array[i]} = ibm-bai-operator-catalog-subscription* || ${sub_array[i]} = ibm-bai-foundation-operator-v24.0-ibm-bai-operator-catalog* ]]; then
        ...
        for i in ${!sub_array[@]}; do
            if [[ ! -z "${sub_array[i]}" ]]; then
                if [[ ${sub_array[i]} = ibm-bai-operator-catalog-subscription* || ${sub_array[i]} = ibm-bai-foundation-operator-v24.0-ibm-bai-operator-catalog* ]]; then
                    ${CLI_CMD} patch subscription.operators.coreos.com ${sub_array[i]} -n $TARGET_PROJECT_NAME -p '{"spec":{"channel":"v24.0"}}' --type=merge >&3 2>&3
                    if [ $? -eq 0 ]
                    then
        ...
        if [[ $ENABLE_PRIVATE_CATALOG -eq 0 ]]; then
            # No longer required to check for Postgres upgrade sub logic, upgrading CPFS is sufficient
        ...
        # Calling function to see if the operator version is the right version
        # the valid_bai_operator_version is set to true if it meets the condition in check_valid_bai_operator_version in common.sh
        valid_bai_operator_version=false
        check_valid_bai_operator_version $bai_operator_csv_version
        ...
        if [[ $READY_FOR_DIRECT_UPGRADE == "Yes" ]]; then
            ...
            if [[ "$bai_operator_csv_version" == "24.0."* ]]; then
                ...
                if [[ $UPGRADE_MODE == "dedicated2dedicated" && $ENABLE_PRIVATE_CATALOG -eq 1 ]]; then
                    ...
                    # Tenant script needs to consider the separation of duties scenario before the parameters are passed
                    if [[ $SEPARATE_OPERAND_FLAG == "Yes" ]]; then
                    ...
                    if [ $? -ne 0 ]; then
                        ...
                        echo "           Usage:"
                        echo "           --cpfs-upgrade-mode     : The migration mode for IBM Cloud Pak foundational services. The valid values are [shared2shared/shared2dedicated/dedicated2dedicated]."
                        echo "           --original-bai-csv-ver  : The version of the CSV for the BAI operator before the upgrade. Example value: [24.0.1] for 24.0.0-IF001."
...
if [ "$RUNTIME_MODE" == "upgradeOperatorStatus" ]; then
    ...
    if [[ -z $insightsengine_cr_name ]]; then
        fail "No custom resource found for BAI Standalone under project \"$BAI_SERVICES_NS\"."
        exit 1
    fi
...
if [ "$RUNTIME_MODE" == "upgradeDeployment" ]; then
    ...
    if [[ (! -z $bai_operator_csv_name_target_ns) ]]; then
        ...
        fail "Failed to Find IBM Cloud Pak for Business Automation Operator deployed as AllNamespace mode in the project \"$TARGET_PROJECT_NAME\"."
        exit
    fi
    # Get value of bai_original_csv_ver_for_upgrade_script
    ibm_bai_shared_info_cm=$(${CLI_CMD} get configmap ibm-bai-shared-info --no-headers --ignore-not-found -n $BAI_SERVICES_NS)
...
if [[ "$RUNTIME_MODE" == "upgradeDeploymentStatus" ]]; then
    ...
    function create_custom_idprovider_route() {
        ...
        if [ -a ${TEMP_CP_CONSOLE_FILE_ID_PROVIDER} ]; then
            ...
            ${CLI_CMD} apply -f $TEMP_CP_CONSOLE_FILE_ID_PROVIDER -n $TARGET_PROJECT_NAME_CS >&3 2>&3
            if [ $? -ne 0 ]; then
    ...
    function create_custom_idmgmt_route() {
        ...
        if [ -a ${TEMP_CP_CONSOLE_FILE_ID_MGMT} ]; then
            ...
            ${CLI_CMD} apply -f $TEMP_CP_CONSOLE_FILE_ID_MGMT -n $TARGET_PROJECT_NAME_CS >&3 2>&3
            if [ $? -ne 0 ]; then
    ...
    if [[ ! -z "$zen_service_name" ]]; then
        ...
        if [[ "$bai_original_csv_ver_for_upgrade_script" == "24.0."* ]]; then
            ...
            if [[ ! -z $tmp_val_result ]]; then
                ${CLI_CMD} delete job create-postgres-license-config -n $bai_services_namespace >&3 2>&3
            fi
            ...
            if [[ ! -z $tmp_val_result ]]; then
                ${CLI_CMD} delete pod -l name=operand-deployment-lifecycle-manager -n $bai_services_namespace >&3 2>&3
            fi

            tmp_val_result=$(${CLI_CMD} get job create-postgres-license-config -n $bai_operators_namespace --no-headers --ignore-not-found)
            if [[ ! -z $tmp_val_result ]]; then
                ${CLI_CMD} delete job create-postgres-license-config -n $bai_operators_namespace >&3 2>&3
            fi
            ...
            if [[ ! -z $tmp_val_result ]]; then
                ${CLI_CMD} delete pod -l name=operand-deployment-lifecycle-manager -n $bai_operators_namespace >&3 2>&3
            fi
        fi
