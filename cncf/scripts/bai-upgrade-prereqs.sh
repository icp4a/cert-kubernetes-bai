...
function check_prereqs() {
    ...
    if [[ "$vcs" == "unknown" ]]; then
        ...
        error "Detected common services version ${vcs} in namespace ${bai_namespace} which is not greater or equals to version ${cs_minimal_version_for_ifix}, are you upgrading from a 24.0.0 version?"
        exit 1
    elif [[ $(semver_compare ${vcs} ${cs_maximal_version_for_ifix}) != "-1" ]]; then
        error "Detected common services version ${vcs} in namespace ${bai_namespace} which is not lower to version ${cs_maximal_version_for_ifix}, are you upgrading from a 24.0.0 version?"
        exit 1
    else
...
function check_subscription() {
    ...
    if [ "${channel}" = "${bai_channel}" ]; then
        ...
        error "Cannot find BAI subscription in namespace ${bai_namespace} or its channel is not ${bai_channel}. Are you upgrading for an ifix of the same BAI version?"
        exit 1
    fi
}
...
function upgrade_to_ifix() {
    ...
    upgrade_bai_subscription ${bai_channel} ${bai_channel} # keep same channel 
}
